...
Instructions:
1. Find ALL self-storage facilities within {req.radius} miles of {location}.
...
async def run_fact_check(req: FactCheckRequest, request: Request, db: Session = Depends(get_db)):
    email = get_current_user(request)
    user = db.query(User).filter(User.email == email).first()
    if not user:
        raise HTTPException(404, "User not found")
    ...
    comps_file = os.path.join(OUTPUT_DIR, safe_name)
    if not os.path.exists(comps_file):
        raise HTTPException(404, "Comps file not found")

    _log_usage(user.id, "fact_check", safe_name, db)
